...
class CorporateActionGenericRouter(Generic[ModelType, CreateSchemaType, ResponseSchemaType]):
    def __init__(
        ...
        # Get common fields from Base and specific fields from the model
        self.base_fields = _get_model_fields(self.base_model)
    ...
    def _get_specific_fields(self) -> List[str]:
        """Get fields that are specific to the  type (not in Base)"""
        model_fields = _get_model_fields(self.model)
        return [field for field in model_fields if field not in self.base_fields]
    ...
    def _separate_fields(self, data: Dict) -> (Dict, Dict):
        """Separate fields into Base fields and specific  type fields"""
        base_data = {}
        specific_data = {}
    ...
    async def create_item(self, db: db_dependency, item: CreateSchemaType):
        try:
            ...
            # Create a new instance of the specific model
            db_item = self.model()

            # Separate and set base fields and specific fields
            base_data, specific_data = self._separate_fields(item_data)
    ...
    async def partial_update_item(self, item_id: str, updated_item: CreateSchemaType, db: db_dependency):
        ...
        try:
            ...
            # Separate and update base fields and specific fields
            base_data, specific_data = self._separate_fields(update_data)
