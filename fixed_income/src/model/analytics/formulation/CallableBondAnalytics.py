...
from QuantLib import BondFunctions, BondPrice, Callability, CallabilitySchedule, CallableFixedRateBond, \
    ...
    Simple, SimpleQuote, SobolRsg, TimeGrid, \
...
from fixed_income.src.model.bonds import CallableBondModel
...
class CallableBondAnalytics(BondAnalyticsBase):
    ...
    def _validate_inputs(self) -> None:
        ...
        if self.call_schedule is None:
            raise ValueError("Call schedule required for Callable Bond")
    ...
    def _filter_call_schedule(self) -> List[Dict]:
        """Filter out historical call dates and validate remaining schedule."""
        ...
        for entry in self.call_schedule:
            if not isinstance(entry, dict) or "date" not in entry or "price" not in entry:
                raise ValueError("Call schedule entries must be dicts with 'date' and 'price' keys")

            call_date = to_ql_date(date.fromisoformat(entry["date"]))
            adjusted_call_date = self.calendar.adjust(call_date, self.business_day_convention)

            # Only include call dates that are in the future relative to evaluation date
            if adjusted_call_date > self.evaluation_date:
                filtered_schedule.append({
                    "date": adjusted_call_date,
    ...
    def _build_callability_schedule(self):
        """Build callability schedule considering only future call dates."""
        filtered_schedule = self._filter_call_schedule()

        if not filtered_schedule:
            logging.warning("No valid future call dates found in call schedule")
            return
    ...
    def build_quantlib_bond(self,
        ...
        if self._bond is None:
            ...
            model = HullWhite(self._discount_curve, mean_reversion, volatility)

            years_to_maturity = self._discount_curve.referenceDate().yearFraction(self.maturity_date)
            time_grid = TimeGrid(years_to_maturity, time_steps)
    ...
    def yield_to_call(self) -> float:
        ...
        try:
            ...
            # Find the first call date strictly after eval date
            future_calls = [c for c in self.callability_schedule if c.date() > self.evaluation_date]
            if not future_calls:
    ...
    def call_probability(self, num_paths: int = 1000) -> Dict[date, float]:
        ...
        """
        try:
            if not self.callability_schedule:
                return {}

            # Get future call dates
            future_calls = [
                (c.date(), c.price().amount())
                for c in self.callability_schedule
    ...
    def _generate_interest_rate_path(self, model, end_date, sequence_generator):
        ...
        for call_date in self.callability_schedule:
