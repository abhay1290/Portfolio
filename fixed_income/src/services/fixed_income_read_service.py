...
class BondReadOnlyService:
    ...
    async def get_bond_by_id(self, bond_id: int, bond_type: str) -> Optional[Any]:
        """
        Get bond by ID with future caching support.
        """
        try:
            # Future: Check cache first
            # cached = await self.cache.get(f"bond:{bond_type}:{bond_id}")
            ...
            # if bond: await self.cache.setex(f"bond:{bond_type}:{bond_id}", 3600, bond.json())

            return bond
        except Exception as e:
            logger.error(f"Error getting {bond_type} bond by ID {bond_id}: {str(e)}")
            return None

    async def get_bond_by_symbol(self, symbol: str, bond_type: str) -> Optional[Any]:
        """
        Get bond by symbol with future caching support.
        """
        try:
            # Future: Check cache first
            # cached = await self.cache.get(f"bond:symbol:{bond_type}:{symbol}")
            # if cached: return ResponseSchema.parse_raw(cached)
            ...
            # Future: Cache the result
            # if bond: await self.cache.setex(f"bond:symbol:{bond_type}:{symbol}", 3600, bond.json())

            return bond
        except Exception as e:
            logger.error(f"Error getting {bond_type} bond by symbol {symbol}: {str(e)}")
            return None

    async def get_bonds_bulk(self, bond_ids: List[int], bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error in bulk {bond_type} bond retrieval: {str(e)}")
            return []

    async def get_bonds_by_symbols_bulk(self, symbols: List[str], bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error in bulk symbol retrieval for {bond_type}: {str(e)}")
            return []
    ...
    async def get_all_bonds(
        ...
        try:
            ...
            logger.error(f"Error getting all {bond_type} bonds: {str(e)}")
            return []

    # === Filtered Read Operations ===

    async def get_active_bonds(self, bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting active {bond_type} bonds: {str(e)}")
            return []

    async def get_bonds_by_issuer(self, issuer: str, bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting {bond_type} bonds by issuer {issuer}: {str(e)}")
            return []

    async def get_bonds_by_currency(self, currency: str, bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting {bond_type} bonds by currency {currency}: {str(e)}")
            return []
    ...
    async def get_bonds_by_maturity_range(
        ...
        try:
            ...
            query = f"""
                SELECT * FROM {bond_type.lower()}_bonds 
            ...
            logger.error(f"Error getting {bond_type} bonds by maturity range: {str(e)}")
            return []

    # === Search Operations ===

    async def search_bonds(self, search_term: str, bond_type: str) -> List[Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error searching {bond_type} bonds with term '{search_term}': {str(e)}")
            return []

    # === Validation Operations ===

    async def validate_bond_exists(self, bond_id: int, bond_type: str) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Error validating {bond_type} bond {bond_id}: {str(e)}")
            return False

    async def validate_bonds_exist(self, bond_ids: List[int], bond_type: str) -> Dict[int, bool]:
        """
        ...
        try:
            ...
            logger.error(f"Error validating {bond_type} bonds exist: {str(e)}")
            return {bond_id: False for bond_id in bond_ids}

    async def validate_symbol_unique(self, symbol: str, bond_type: str, exclude_id: int = None) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Error validating symbol uniqueness {symbol} for {bond_type}: {str(e)}")
            return False

    # === Statistics and Analytics ===

    async def count_bonds(self, bond_type: str, **filters) -> int:
        """
        ...
        try:
            ...
            logger.error(f"Error counting {bond_type} bonds: {str(e)}")
            return 0

    async def get_bond_summary_stats(self, bond_type: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting {bond_type} bond summary stats: {str(e)}")
            return {
                "bond_type": bond_type,
                "total_bonds": 0,
                "active_bonds": 0,
    ...
    async def get_all_bond_types_summary(self) -> Dict[str, Any]:
        ...
        """
        bond_types = ['FIXED_COUPON', 'ZERO_COUPON', 'CALLABLE', 'PUTABLE', 'FLOATING', 'SINKING_FUND']
        summary = {}

        for bond_type in bond_types:
            try:
                summary[bond_type] = await self.get_bond_summary_stats(bond_type)
            except Exception as e:
                logger.error(f"Error getting summary for {bond_type}: {str(e)}")
                summary[bond_type] = {"error": str(e)}

        return summary

    # === Batch Processing for Performance ===

    async def get_current_prices(self, bond_ids: List[int], bond_type: str) -> Dict[int, float]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting current prices for {bond_type}: {str(e)}")
            return {}

    async def get_bond_essentials(self, bond_ids: List[int], bond_type: str) -> Dict[int, Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting {bond_type} bond essentials: {str(e)}")
            return {}

    # === Future Cache Management Methods ===

    # async def invalidate_cache(self, bond_id: int, bond_type: str):
    #     """Invalidate cache for specific bond"""
    #     await self.cache.delete(f"bond:{bond_type}:{bond_id}")

    # async def invalidate_symbol_cache(self, symbol: str, bond_type: str):
    #     """Invalidate cache for specific symbol"""
    #     await self.cache.delete(f"bond:symbol:{bond_type}:{symbol}")

    # async def warm_cache(self, bond_ids: List[int], bond_type: str):
    #     """Pre-warm cache with frequently accessed bonds"""
    #     bonds = await self.get_bonds_bulk(bond_ids, bond_type)
    #     for bond in bonds:
    #         await self.cache.setex(f"bond:{bond_type}:{bond.id}", 3600, bond.json())
